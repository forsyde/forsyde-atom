...
\begin{tikzpicture}[nomoccolor, type style = \scriptsize\texttt]
  ...
  \primitive[]    {p3}{0,-2}{$\scanS$};
  \vector[] (p1.center) -> (p2);
  \vector[token=(scalar), token pos=.6] (p2.center) - (0,-1);
  \vector[token=vector] (p3.center) -> (1.5,-2);
  \primitive[f=pc]{p1}{0,1} {$\fMapS$};
  \primitive[]    {p2}{0,0} {$\scanS$};
  \primitive[]    {p3}{0,-2}{$\scanS$};
...
\newsavebox\network
\begin{lrbox}{\network}
\begin{tikzpicture}[nomoccolor, type style = \scriptsize\texttt, global scale =.9]
   \pgfmathsetmacro\c{2}
   ...
   \node[anchor=east] (v1) at ($(s11)+(0,1)$) {$\vect{s}_1$};
   ...
   \node[anchor=east] (v2) at ($(e)!.5!(g)$) {$\vect{s}_2$};
   % \vector[] (v2) -> ($(e)!.5!(g)$);

   \vector[] ($(p11-p.i2)!.5!(p21-p.i2)$) -> ($(p12-p.i2)!.5!(p22-p.i2)+(4.8,0)$);
   \vector[] ($(p21-p.i2)+(1,-1)$) -> ($(p22-p.i2)+(1,-1)+(2.8,0)$);
   \vector[] ($(p12-p.i2)!.5!(p22-p.i2)+(4.8,0)$) - ($(p12-p.i2)!.5!(p22-p.i2)+(4.8,-4)$);
