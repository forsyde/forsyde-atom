...
eventToSY (DE.DE t a) = (SY.SY (pure t), SY.SY a)


argumentToSY :: DE.DE (DE.DEArg c a) -> (SY.SY (SY.SYArg c (Value Tag)), SY.SY (SY.SYArg c a))
argumentToSY (DE.DE t a) = (SY.SY ((pure.pure) t), SY.SY (SY.SYArg (DE.unArg a)))
...
toSY2 s1 s2        = let (tags, sigs)         = (argumentToSY <$> ((psi21 . psi21) (,) -$- s1 -*- s2) |<)
                         (sy1, sy2)           = (sigs ||||<)
                     in  (o tags, o sy1, o sy2) 
toSY3 s1 s2 s3     = let (tags, sigs)         = (argumentToSY <$> ((psi31 . psi31) (,,) -$- s1 -*- s2 -*- s3) |<)
                         (sy1, sy2, sy3)      = (sigs ||||<<)
                     in  (o tags, o sy1, o sy2, o sy3) 
toSY4 s1 s2 s3 s4  = let (tags, sigs)         = (argumentToSY <$> ((psi41 . psi41) (,,,) -$- s1 -*- s2 -*- s3 -*- s4) |<)
