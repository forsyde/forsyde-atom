...
import           ForSyDe.Atom.MoC.SY.Core (part, stream)
...
test_when = SY.when p s1 @?= stream [Abst, Abst, Abst, Value 4, Value 5]
  where p = SY.comb11 (>3) s1

test_filter = SY.filter (>3) s1 @?= stream [Value 1, Value 2, Value 3, Undef, Undef]

test_fill = SY.fill 5 s @?= stream [Value 1, Value 2, Value 3, Value 5, Value 5]
  where s = SY.filter (>3) s1

test_hold = SY.hold 5 s @?= stream [Value 1, Value 2, Value 3, Value 3, Value 3]
  where s = SY.filter (>3) s1
...
instance (Ord a) => Ord (SY.SY [a]) where
  compare (SY.SY [x]) (SY.SY [y]) = compare x y
