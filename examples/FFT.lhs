...
-- assumes n == 2 ^ k
fft bffunc k vs = (V.bitrev . (stage `V.pipe1` widths)) vs
  where
    n = V.length vs 
    stage     w         = V.concat . (segment `V.map21` (V.take (n `div` w) twiddles)) . V.group w
    segment   twid      = V.unduals . ((butterfly twid) `V.map11`) . V.duals
    butterfly w (s1,s2) = (bffunc w) `MoC.comb22` s1 $ s2
    widths              = V.iterate k (*2) 2
    twiddles            = (V.bitrev . (halfcycle `V.map11`) . V.take (n `div` 2)) V.indexes
      where halfcycle l = (cis . negate) $
                          -2 * pi * (fromInteger (l - 1)) / (fromInteger n)
